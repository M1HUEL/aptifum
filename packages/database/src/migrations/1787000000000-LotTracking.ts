...
export class LotTracking1787000000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE UNIQUE INDEX "UQ_product_lots_tenant_product_warehouse_lot" ON "product_lots" ("tenant_id", "product_id", "warehouse_id", "lot_number") WHERE "variant_id" IS NULL`,
    );
    await queryRunner.query(
      `CREATE UNIQUE INDEX "UQ_product_lots_tenant_variant_warehouse_lot" ON "product_lots" ("tenant_id", "product_id", "variant_id", "warehouse_id", "lot_number") WHERE "variant_id" IS NOT NULL`,
    );
    await queryRunner.query(`CREATE INDEX "IDX_stock_movements_lot_id" ON "stock_movements" ("lot_id") `);
    await queryRunner.query(
      `ALTER TABLE "product_lots" ADD CONSTRAINT "FK_product_lots_product" FOREIGN KEY ("product_id") REFERENCES "products"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE "product_lots" ADD CONSTRAINT "FK_product_lots_variant" FOREIGN KEY ("variant_id") REFERENCES "product_variants"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE "product_lots" ADD CONSTRAINT "FK_product_lots_warehouse" FOREIGN KEY ("warehouse_id") REFERENCES "warehouses"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
  }
