...
export class F2Accounting1785892859876 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."chart_accounts_type_enum" AS ENUM('asset', 'liability', 'equity', 'revenue', 'expense')`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "journal_entry_lines" ADD CONSTRAINT "FK_jel_entry" FOREIGN KEY ("entry_id") REFERENCES "journal_entries"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE "journal_entry_lines" ADD CONSTRAINT "FK_jel_account" FOREIGN KEY ("account_id") REFERENCES "chart_accounts"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE "journal_entries" ADD CONSTRAINT "FK_je_period" FOREIGN KEY ("period_id") REFERENCES "accounting_periods"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
  }
